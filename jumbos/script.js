...
function filterAndDisplay(pitcherName) {
    ...
    pitchTypes.forEach(pitch => {
        ...
        // Calculate average spin rate
        const spinRates = pitches.map(p => parseFloat(p.spin_rate)).filter(v => !isNaN(v));
        const spin = spinRates.length > 0
        ? Math.round(spinRates.reduce((a, b) => a + b, 0) / spinRates.length)
        : "N/A";
    ...
    const headers = [
        "Pitch Type", "Count", "%", "Max Velo", "Min Velo", "Avg Velo", "Spin Rate",
        "1-1", "0-2/1-2", "2-2/3-2"
    ];      
    let html = "<table><thead><tr>";
    headers.forEach(header => {
